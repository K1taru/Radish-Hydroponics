...
#define SIM_NORMAL_DURATION     3600000UL
#define SIM_LOW_TDS_DURATION    30000UL
#define SIM_LOW_WATER_DURATION  20000UL

enum SimState { SIM_NORMAL, SIM_LOW_TDS, SIM_LOW_WATER };
SimState simState = SIM_NORMAL;
unsigned long simStateStart = 0;
unsigned long simCycleCount = 0;
...
void runSimulation(unsigned long now) {
  if (simStateStart == 0) {
    simStateStart = now;
    simState = SIM_NORMAL;
    Serial.println(F("=== SIMULATION MODE ACTIVE ==="));
    Serial.println(F("Cycle: Normal -> Low TDS -> Normal -> Low Water -> Repeat"));
  }
  ...
  switch (simState) {
    case SIM_NORMAL:
      ...
      if (elapsed >= SIM_NORMAL_DURATION) {
        if (simCycleCount % 2 == 0) {
          simState = SIM_LOW_TDS;
          Serial.println(F("[SIM] Switching to LOW TDS state - will trigger nutrient dosing"));
        } else {
          simState = SIM_LOW_WATER;
          Serial.println(F("[SIM] Switching to LOW WATER state - will trigger refill"));
        }
        simStateStart = now;
        simCycleCount++;
      }
    ...
    case SIM_LOW_TDS:
      ...
      if (mixState == MIX_NONE && elapsed >= SIM_LOW_TDS_DURATION) {
        simState = SIM_NORMAL;
        simStateStart = now;
        Serial.println(F("[SIM] Returning to NORMAL state"));
      }
      break;
...
void readSensors() {
  if (SIMULATION_MODE) {
    ...
    static unsigned long lastSimDebug = 0;
    if (millis() - lastSimDebug > 5000) {
      Serial.print(F("[SIM] State: "));
      switch (simState) {
        case SIM_NORMAL:    Serial.print(F("NORMAL")); break;
        case SIM_LOW_TDS:   Serial.print(F("LOW_TDS")); break;
        case SIM_LOW_WATER: Serial.print(F("LOW_WATER")); break;
      }
      Serial.print(F(" | pH: ")); Serial.print(pH, 1);
      Serial.print(F(" | TDS: ")); Serial.print((int)tds);
