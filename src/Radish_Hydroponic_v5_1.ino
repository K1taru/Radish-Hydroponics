...
// Manual override mode (workaround for bad sensor readings)
bool phManualMode = false;
bool tdsManualMode = false;
float phManualValue = 6.0;
float tdsManualValue = 750.0;
...
void setup() {
  ...
  Serial.println(F("Manual Override Commands:"));
  Serial.println(F("  PHMAN <value>  - Set manual pH (e.g. PHMAN 6.5)"));
  Serial.println(F("  TDSMAN <value> - Set manual TDS (e.g. TDSMAN 750)"));
  Serial.println(F("  PHAUTO         - Return to pH sensor"));
  Serial.println(F("  TDSAUTO        - Return to TDS sensor"));
  Serial.println(F("  STATUS         - Show current mode"));
}
...
void checkSerialCommands() {
  if (Serial.available() > 0) {
    ...
    if (cmd.startsWith("PHMAN ")) {
      float val = cmd.substring(6).toFloat();
      if (val >= 0.0 && val <= 14.0) {
        phManualValue = val;
        phManualMode = true;
        Serial.print(F("pH manual mode ON: "));
        Serial.println(val, 2);
      } else {
        Serial.println(F("Invalid pH value (0-14)"));
      }
    }
    else if (cmd.startsWith("TDSMAN ")) {
      float val = cmd.substring(7).toFloat();
      if (val >= 0.0 && val <= 5000.0) {
        tdsManualValue = val;
        tdsManualMode = true;
        Serial.print(F("TDS manual mode ON: "));
        Serial.println((int)val);
      } else {
        Serial.println(F("Invalid TDS value (0-5000)"));
      }
    }
    else if (cmd == "PHAUTO") {
      phManualMode = false;
      Serial.println(F("pH auto mode ON (sensor)"));
    }
    else if (cmd == "TDSAUTO") {
      tdsManualMode = false;
      Serial.println(F("TDS auto mode ON (sensor)"));
    }
    else if (cmd == "STATUS") {
      Serial.println(F("=== Override Status ==="));
      Serial.print(F("pH Mode: "));
      if (phManualMode) {
        Serial.print(F("MANUAL ("));
        Serial.print(phManualValue, 2);
        Serial.println(F(")"));
      } else {
        Serial.println(F("AUTO (sensor)"));
      }
      Serial.print(F("TDS Mode: "));
      if (tdsManualMode) {
        Serial.print(F("MANUAL ("));
        Serial.print((int)tdsManualValue);
...
void updateOLED() {
  ...
  if (phSensorError) {
    ...
    oled.print(pH, 2);
    if (phManualMode) oled.print(F("M"));  // M = Manual
    else              oled.print(F(" "));
  ...
  if (tdsSensorError) {
    ...
    oled.print((int)tds);
    if (tdsManualMode) oled.print(F("M ppm "));  // M = Manual
    else               oled.print(F(" ppm "));
