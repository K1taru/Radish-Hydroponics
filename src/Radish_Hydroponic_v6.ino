...
// Calibration
// pH calibration: m = (pH7 - pH4) / (Vph7 - Vph4)
// Example: m = (7 - 4.01) / (2.5 - 3.05) = -5.436
// Calibrate with pH 7.0 buffer: adjust RV1 until Po = 2.50V
// Then measure Po with pH 4.01 buffer and calculate your own 'm'
#define PH_SLOPE          -5.70   // Default slope (adjust after calibration with pH 4.01 buffer)
#define PH_V_AT_7         2.50    // Voltage at pH 7.0 (calibrated with RV1 potentiometer)
#define TDS_K             1.0
#define VREF              5.0
...
void readSensors() {
  ...
  float Po = phRaw * (VREF / 1024.0);  // Po = voltage at pH sensor output
  ...
  if (millis() - lastpHDebug > 10000) {
    ...
    Serial.print(Po, 3);
    Serial.print(F("V, pH: "));
    // pH formula from calibration: pH = 7 - (2.5 - Po) * m
    Serial.println(7.0 - (PH_V_AT_7 - Po) * PH_SLOPE, 2);
    lastpHDebug = millis();
  }
  ...
  if (Po < 0.2 || Po > 4.8) {
    phSensorError = true;
    Serial.print(F("pH sensor error! Po: "));
    Serial.print(Po, 3);
    Serial.println(F("V - Check connection."));
    // Keep last valid pH reading
  } else {
    phSensorError = false;
    // pH calculation using calibration formula:
    // pH = 7 - (Vph7 - Po) * m
    // Where m = slope from two-point calibration (pH 7 and pH 4.01)
    pH = constrain(7.0 - (PH_V_AT_7 - Po) * PH_SLOPE, 0.0, 14.0);
  }
