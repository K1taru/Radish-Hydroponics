 ...
 * 
 * ==================== VERSION 2.0 CHANGES ====================
 * 1. SEQUENTIAL DOSING WITH MIXING PREVENTION:
 *    - Solution A and Solution B are now dosed sequentially with water pump
 *      mixing cycles in between to prevent direct mixing of concentrates
 *    - Algorithm: Dose A → Mix with water pump → Dose B → Mix again
 *    - This prevents calcium (in A) from precipitating with phosphate/sulfate (in B)
 * 
 * 2. NEW MIXING CYCLE ALGORITHM:
 *    - After dosing Nutrient A: Water pump forced ON for MIXING_DURATION_A (3 min)
 *    - After mixing A: Dose Nutrient B
 *    - After dosing Nutrient B: Water pump forced ON for MIXING_DURATION_B (5 min)
 *    - Mixing cycles override normal pump schedule temporarily
 *    - After final mixing, system returns to normal pump schedule
 * 
 * 3. STATE MACHINE FOR MIXING:
 *    - Added mixingState variable to track dosing/mixing sequence
 *    - States: NONE, DOSING_A, MIXING_A, DOSING_B, MIXING_B
 *    - Prevents interference with normal irrigation cycles
 * 
 * 4. ENHANCED SAFETY:
 *    - Water pump stays on during entire mixing sequence
 *    - Ensures thorough dilution before next chemical addition
 *    - Prevents clogging and precipitation in reservoir
 * 
 * RATIONALE:
 * Without a mechanical mixer, using the water pump circulation provides
 * the mixing action needed to safely dilute each nutrient component before
 * adding the next, preventing chemical reactions in concentrated form.
 * ============================================================
 */
