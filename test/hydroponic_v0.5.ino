...
// NOTE: pH adjustment is manual in this version (no pH pumps)
...
void checkAndAdjustNutrients(unsigned long currentMillis) {
  ...
  // pH monitoring only - manual adjustment required
  if (currentPH < (PH_MIN + PH_TOLERANCE)) {
    Serial.print(F("MANUAL ACTION: pH low ("));
    Serial.print(currentPH, 2);
    Serial.println(F(") - Add pH UP solution manually"));
    systemStatus = "pH LOW-Manual";
  } else if (currentPH > (PH_MAX - PH_TOLERANCE)) {
    Serial.print(F("MANUAL ACTION: pH high ("));
    Serial.print(currentPH, 2);
    Serial.println(F(") - Add pH DOWN solution manually"));
    systemStatus = "pH HIGH-Manual";
  }
}
...
void updateLCD() {
  ...
  if (currentPH >= PH_MIN && currentPH <= PH_MAX) {
    ...
    lcd.print("ADD UP");  // Manual instruction: Add pH UP
  } else {
    lcd.print("ADD DOWN");  // Manual instruction: Add pH DOWN
  }
